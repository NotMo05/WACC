...
object ExprGen {
  ...
  // Replace divMod method in ExprGen.scala with this optimized version
  def divMod(l: Expr, r: Expr, regNum: Int, reg: RegName, asmBuilder: Builder[Instr, List[Instr]]) = {
    // Save RAX and RDX which are modified by div instruction
    asmBuilder += PUSH(Reg(Rax, QWord))
    asmBuilder += PUSH(Reg(Rdx, QWord))
    
    // Generate code for left operand directly into RAX
    exprGenRegister(l, asmBuilder, regNum) 
    asmBuilder += MOV(Reg(Rax, DWord), Reg(regNum, DWord))
    
    // Generate code for right operand and check for division by zero
    val rightReg = exprGenRegister(r, asmBuilder, regNum)
    asmBuilder += CMP(rightReg, Imm(0))
    asmBuilder += JCond(Cond.E, Label("_errDivZero"))
    
    // Perform division
    asmBuilder += CDQ
    asmBuilder += IDIV(Reg(regNum, DWord))
    
    // Move quotient or remainder to result register
    asmBuilder += MOV(Reg(regNum, DWord), Reg(reg, DWord))
    
    // Restore saved registers in reverse order
    asmBuilder += POP(Reg(Rdx, QWord))
    asmBuilder += POP(Reg(Rax, QWord))
